...
def _remap_hypnogram_for_plotting(hyp):
    ...
    hyp_plot = np.array(hyp).copy().astype(float)
...
def plot_hypnodensity(hyp, ypred, raw, nclasses=6, figoutdir='./', filename='dashboard.png', titl="ezscore-f", scorer_type='forehead'):
    ...
    try:
        ...
        if nclasses == 5:
            ...
            plot_order_indices = list(range(nclasses))
            prob_plot_labels = [f"Class {i}" for i in range(nclasses)]
        ...
        try:
            ...
            palette = sns.color_palette("viridis", n_colors=nclasses)
